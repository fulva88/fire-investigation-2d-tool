...
- 패치노트 팝업 동작: 새 버전을 열면 최소 한 번은 반드시 뜸. "오늘 하루 보지 않기"를
  ...
  버전 기록 누적 열람 가능.
- **패치노트(팝업 항목 + txt 파일)를 작성하기 직전에는 항상 먼저 실제 현재 날짜를 확인할 것.**
  대화 앞부분에서 이미 알고 있던 날짜를 그대로 재사용하지 말고, 그 시점에 확인 가능한
  가장 신뢰할 수 있는 소스(시스템 reminder의 currentDate, 또는 테스트 중 브라우저에서
  `new Date()`로 직접 확인)로 다시 체크할 것. (과거 v1.1에서 세션 앞부분 날짜를 그대로
  재사용했다가 실제 날짜와 이틀 차이가 나서 사용자가 지적한 적 있음 — 세션이 길어지거나
  중간에 시간이 흐르면 날짜가 조용히 바뀔 수 있으므로, 매 패치마다 예외 없이 다시 확인.)
